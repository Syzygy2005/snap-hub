...
export function DataFreshness({ checkedAt, archived = false }: { checkedAt: string | null; archived?: boolean }) {
  const now = useNow();
  if (archived) return <p className="mb-4 text-sm text-muted">Archived season · these standings no longer refresh.</p>;
  const delayed = checkedAt && now !== null && now - new Date(checkedAt).getTime() > 30 * 60_000;
  return <p role="status" className={`mb-4 rounded-lg border border-line px-3 py-2 text-sm ${delayed ? "text-gold" : "text-muted"}`}>
