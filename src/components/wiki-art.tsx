...
export function WikiArt({ name, art, featured = false }: { name: string; art: string; featured?: boolean }) {
  ...
  const image = <div className="relative">
    {art && !failed ? /* eslint-disable-next-line @next/next/no-img-element */
      <img src={artSrc(art)} alt={name} onError={() => setFailed(true)} loading={featured ? "eager" : "lazy"} decoding="async" draggable={false} className="aspect-square w-full object-contain" />
