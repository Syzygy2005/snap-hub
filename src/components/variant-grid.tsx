...
export function VariantGrid({items,anchors=false,controls=true}: {items:VariantEntry[];anchors?:boolean;controls?:boolean}) {
  return <div className="grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4">{items.map(({card_id,card_name,variant:v})=><article key={`${card_id}:${v.id}`} id={anchors?`variant-${encodeURIComponent(v.id)}`:undefined} className="min-w-0 scroll-mt-48 rounded-xl border border-line bg-surface/40 p-3">
    <a href={artSrc(v.art)} target="_blank" rel="noreferrer" aria-label={`View ${card_name} variant ${v.id} artwork`}><WikiArt name={`${card_name} variant ${v.id}`} art={v.art}/></a>
    ...
    <p className="mt-2 flex flex-wrap gap-1.5">{[v.rarity,v.collectorQuality].filter(Boolean).map((label,i) => <span key={i} className="status-badge text-accent">{label}</span>)}{!v.rarity && !v.collectorQuality && <span className="text-xs text-muted">Category not listed</span>}</p>
    {v.status==="unreleased" && <p className="mt-2"><span className="status-badge text-accent">Unreleased preview</span></p>}
