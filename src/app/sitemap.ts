...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  return ["/", "/wiki", "/wiki/cards", "/wiki/locations", "/wiki/variants", "/wiki/history", ...WIKI_GUIDES.map(g=>`/wiki/${g.slug}`), ...collections.flatMap((items,i) => items.filter(e => e.status === "released").map(e => `/wiki/${i === 0 ? "cards" : "locations"}/${encodeURIComponent(e.def_id)}`))].map(path => ({url: origin+path}));
}
