...
import { clampPage, cleanQuery, value } from "@/lib/wiki/filter";
...
export default async function CollectionPage(props:PageProps<"/wiki/my-collection">) {
  ...
  if(!account)return <><PageHeader title="Your collection, your taste." subtitle="Keep track of artwork you own and variants you want next."/>{discordConfig()?<a href="/api/auth/discord?return=%2Fwiki%2Fmy-collection" className="inline-block rounded bg-jade px-5 py-3 font-bold text-forest">Sign in with Discord</a>:<p>Collection sign-in is not available on this installation.</p>}<p className="mt-4 text-sm text-muted">Your collection is private until you choose to share a wishlist.</p></>;
  const sp=await props.searchParams,all=await collection(account.id),filter=value(sp,"status"),q=cleanQuery(value(sp,"q")).toLowerCase();
  ...
  const {page,pages}=clampPage(value(sp,"page"),items.length,36),visible=items.slice((page-1)*36,page*36);
  return <><PageHeader title="My variant collection" subtitle="A manual checklist of your artwork. Changes here do not change your in-game collection."/>
    ...
    <Pager label="Collection pages" base={"/wiki/my-collection"} sp={sp} page={page} pages={pages}/>
  </>;
}
