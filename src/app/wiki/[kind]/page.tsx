...
export default async function Gallery(props: PageProps<"/wiki/[kind]">) {
  ...
  return <>
    ...
    <form className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-line bg-surface p-4" action={`/wiki/${kind}`}>
      ...
      <button className="brand-action hover:brightness-110 rounded bg-jade px-4 py-2 text-sm font-bold text-forest">Search</button><Link className="py-2 text-sm text-muted underline" href={`/wiki/${kind}`}>Reset</Link>
    </form>
