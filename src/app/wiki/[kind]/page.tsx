...
export default async function Gallery(props: PageProps<"/wiki/[kind]">) {
  ...
  return <>
    ...
    <form className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border border-line bg-surface p-4" action={`/wiki/${kind}`}>
      ...
      <button className="hover:brightness-110 rounded bg-accent px-4 py-2 text-sm font-bold text-bg">Search</button><Link className="py-2 text-sm text-muted underline" href={`/wiki/${kind}`}>Reset</Link>
    </form>
    <p className="mb-4 text-sm text-muted">{result.total} results · Page {result.page} of {result.pages}</p>
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6">{result.items.map(e => <Link key={e.def_id} href={`/wiki/${kind}/${encodeURIComponent(e.def_id)}`} className="hover:border-accent focus-visible:border-accent min-w-0 rounded-lg border border-line bg-surface p-3">
