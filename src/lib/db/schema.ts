...
-- making it. Only an admin can confirm it for public display; until then it is visible to the
-- claimant and admins only. Client-reported tracker names are not ownership proof.
create table if not exists player_claims (
  player_id int primary key references players(id) on delete cascade,
  account_id int not null references accounts(id) on delete cascade,
  claimed_at timestamptz not null default now(),
  verified_at timestamptz,
  -- Only 'admin' is trusted. Legacy 'tracker' rows remain pending in all application reads.
  verified_by text
);
