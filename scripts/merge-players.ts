...
// Wrapped in a function rather than using top-level await: tsx compiles a .ts file here to
// CommonJS, which rejects top-level await outright.
async function main() {
  ...
  const { plan } = result;
  const seasons = (list: { season: string; region: string }[]) =>
    list.map((s) => `${s.season}/${s.region}`).join(", ") || "none";
  ...
  console.log(`  ${plan.seasonsMoved.length} season(s) move as is: ${seasons(plan.seasonsMoved)}`);
  console.log(`  ${plan.seasonsCombined.length} season(s) combine: ${seasons(plan.seasonsCombined)}`);
